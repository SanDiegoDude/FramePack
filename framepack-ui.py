...
import os, argparse, traceback, math
import gradio as gr
import torch, einops, numpy as np, safetensors.torch as sf
...
from diffusers_helper.utils import (
    ...
    resize_and_center_crop, state_dict_weighted_merge,
    state_dict_offset_merge, generate_timestamp
)
...
transformer.to(dtype=torch.bfloat16); vae.to(dtype=torch.float16); image_encoder.to(dtype=torch.float16)
text_encoder.to(dtype=torch.float16); text_encoder_2.to(dtype=torch.float16)
